...
app.frame('/image', (c) => {
  ...
  const state = deriveState(previousState => {
      if (buttonValue === 'next' && inputText) {
        ...
        if (cleanedText.length > 0) previousState.ticker = cleanedText.toUpperCase();
...
app.frame('/end', async (c) => {
  ...
  return c.res({
      ...
      <Button.Link href={`https://basescan.org/token/${tokenAddress}`}>BaseScan</Button.Link>,
...
app.transaction('/create', async (c) => {
  ...
  return c.contract({
      abi: ABI,
      chainId: `eip155:${process.env.CHAIN_ID}` as "eip155:1" | "eip155:10" | "eip155:8453" | "eip155:84532" | "eip155:7777777" | "eip155:666666666",
